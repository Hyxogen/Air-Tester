...
namespace tester {
	namespace event {
		...
		void PrettyResultPrinter::OnTesterStart() {

		}

		void PrettyResultPrinter::OnTesterStop() {

		}

		void PrettyResultPrinter::OnTestGroupStart(TestGroup* testGroup) {
			std::cout << ANSII_COLOR_LIGHT_GREEN << "[---------] " << ANSII_RESET << testGroup->m_Name << std::endl;	
		}

		void PrettyResultPrinter::OnTestStart(Test* test) {
			std::cout << ANSII_COLOR_GREEN << "[ RUN     ] " << ANSII_RESET << test->GetName() << std::endl;
		}

		void PrettyResultPrinter::OnTestGroupFinish(TestGroup* testGroup) {
			(void)testGroup;
			std::cout << ANSII_COLOR_GREEN << "[---------]" << std::endl;
		}

		void PrettyResultPrinter::OnTestFinish(Test* test) {
